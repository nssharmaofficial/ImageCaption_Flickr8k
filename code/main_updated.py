...
if __name__ == '__main__':
    ...
    for epoch in range(0, config.EPOCHS):
        ...
        torch.save(image_encoder.state_dict(), f"checkpoints/NEW_encoder-{config.BATCH}B-{config.HIDDEN_DIM}H-{config.NUM_LAYER}L-e{epoch+1}.pt")
        torch.save(emb_layer.state_dict(), f"checkpoints/NEW_embeddings-{config.BATCH}B-{config.HIDDEN_DIM}H-{config.NUM_LAYER}L-e{epoch+1}.pt")
        torch.save(image_decoder.state_dict(), f"checkpoints/NEW_decoder-{config.BATCH}B-{config.HIDDEN_DIM}H-{config.NUM_LAYER}L-e{epoch+1}.pt")
